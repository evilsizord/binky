...
$BinkyConfig = array(
	...
	array('selector' => 'row', 'template' => '
		<table class="row" border="0" cellpadding="0" cellspacing="0" {{=WIDTH}} {{=STYLE}} {{=ALIGN}}>
		<tbody>
		<tr>
			{{INNERTEXT}}
		</tr>
	...
	array('selector' => 'column', 'template' => '
		<td {{=WIDTH}} {{COLUMN-STYLE}} >{{INNERTEXT}}</td>
	'),

	array('selector' => 'p.default', 'template' => '
		<p style="padding: 0; margin: 10px 0 0 0; font-size: 16px; line-height: 25px; font-family: Helvetica, Arial, sans-serif; color: #333333; text-align: left;">{{INNERTEXT}}</p>
	'),
